...
from univariate_analysis import UnivariateAnalysisStrategy

# =====================================================
# Target Relation Analysis Strategy
# =====================================================
# This strategy analyzes the relationship between a feature and the target variable.


class TargetRelationAnalysisStrategy(UnivariateAnalysisStrategy):
    def __init__(self, target: str):
        """
        Initialize the strategy with the name of the target variable.
    ...
    def analyze(self, df: pd.DataFrame, feature: str):
        ...
        if feature == self.target:
            return  # Skip if feature is the target

        if pd.api.types.is_numeric_dtype(df[feature]):
            plt.figure(figsize=(10, 6))
            sns.boxplot(x=self.target, y=feature, data=df)
            ...
            plt.figure(figsize=(10, 6))
            sns.boxplot(x=feature, y=self.target, data=df)
            plt.title(f"{self.target} by {feature}")
            plt.xlabel(feature)
